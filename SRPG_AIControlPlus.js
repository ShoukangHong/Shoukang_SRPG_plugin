 ...
 *
 * step2Score = step1Score * aiPos[0] + aiPos[1];
 * this step modifies the step1Score with position score.
 *
 ...
 * a : user
 * b : target
 * f.dmg(b) : returns estimated attack damage, this damage already considers attack time.
 * f.time(b) : returns attack time. Can be used for simpler logic.
 * f.hit(b) : gives the probability to hit the target. It considers user hit and target eva. If you change the default hit rule by another plugin, this will not return the right value.
 *
 * *counter Formula*
 * a : user
 * b : target
 * f.dmg(a) : returns estimated counter damage, this damage already considers attack time.
 * f.time(a) : returns counter attack time. Can be used for simpler logic.
 * f.hit(a) : gives the probability to hit the user. If you change the default hit rule by another plugin, this will not return the right value.
 *
 ...
 * f.ntd('type') : nearest type distance, type can be 'friend', 'opponent' (will be converted to 'enemy' or 'friend' depending on user type), 'unitEvent', 'object', etc. if such type doesn't exist, return 100
 * f.nfd('flag') : Nearest flag distance. use with <aiFlag:XXX> event note tag, if such flag doesn't exist, return 100.
 * f.mtd('type') : most type distance, type can be 'friend', 'opponent' (will be converted to 'enemy' or 'friend' depending on user type), 'unitEvent', 'object', etc. if such yype doesn't exist, return 100.
...
(function() {
  ...
  Game_Temp.prototype.clearRangeTable = function() {
	...
	Scene_Map.prototype.srpgAITarget = function(user, event, action, counterScoreList, targetPosList, posModifierTable, randomLevel) {
		...
		targetPosList.forEach(function (targetPos) {
			...
			for (var i = 0; i < userPosList.length; i++) {
				...
				score = score * posModifierTable[pos.x][pos.y][0];
				score += posModifierTable[pos.x][pos.y][1];
				score = score * ratingModifier * (1 + randomLevel * (0.5 - randomGenerator()) * 0.1);
	...
	Game_CharacterBase.prototype.calcSingleTargetScore = function(action, pos, targetEvent, targetScorelist, counterScoreList){
		var score = targetScorelist[targetEvent.eventId()];
		...
		if (counterScore > 0 && !action.item().meta.srpgUncounterable && targetEvent.inCounterRange(pos.x, pos.y)){
			score -= counterScore;
		}
		return score;
	};
	...
	Game_CharacterBase.prototype.counterScore = function(user, counterFormula) {
		...
		var f = {
			dmg : action.srpgPredictionDamageWithAgi.bind(action),
			time : target.getAgiAttackTime.bind(target),
			hit : action.srpgPredictionHitRate.bind(action)
		}
	...
	Game_CharacterBase.prototype.choiceNearestTypeDist = function(type){
		...
		if (type == 'opponent') type = (this.isType() == 'enemy' ? 'actor' : 'enemy');
		var minDist = 100;
...
// preist <aiPos:[(1 + 0.2 * ([2,3].contains(tr) ? (tr - 1) : 0))/(0.03 * f.ntd('opponent') + 0.01 * f.ntd('friend') + 1/(f.ntd('opponent') + 0.1)),
//   (a.mhp/(a.hp + 1)) * ((f.isf('enemyFort') + f.isf('actorFort')) * Math.max(Math.min(a.mhp - 1.2 * a.hp, 0.1 * a.hp), 0) + (f.isf('actorFort') + f.isf('enemyFort') * 0.3 * Math.max(4 - f.ntd('opponent'), 0))*0.12*a.mhp) / (0.03 * f.ntd('opponent') + 0.01 * f.ntd('friend') + 1/(f.ntd('opponent') + 0.1))]>
// mag <aiPos:[(1 + 0.2 * ([2,3].contains(tr) ? (tr - 1) : 0))/(0.06 * f.ntd('opponent') + 0.02 * f.ntd('friend') + 1/(f.ntd('opponent') + 0.2)),
//  (a.mhp/(a.hp + 1)) * ((f.isf('enemyFort') + f.isf('actorFort')) * Math.max(Math.min(a.mhp - 1.2 * a.hp, 0.1 * a.hp), 0) + (f.isf('actorFort') + f.isf('enemyFort') * 0.3 * Math.max(4 - f.ntd('opponent'), 0))*0.12*a.mhp) / (0.06 * f.ntd('opponent') + 0.02 * f.ntd('friend') + 1/(f.ntd('opponent') + 0.2))]>
// arc <aiPos:[(1 + 0.2 * ([2,3].contains(tr) ? (tr - 1) : 0))/(0.09 * f.ntd('opponent') + 0.02 * f.ntd('friend') + 1/(f.ntd('opponent') + 0.2)),
//   (a.mhp/(a.hp + 1)) * ((f.isf('enemyFort') + f.isf('actorFort')) * Math.max(Math.min(a.mhp - 1.4 * a.hp, 0.1 * a.hp), 0) + (f.isf('actorFort') + f.isf('enemyFort') * 0.3 * Math.max(4 - f.ntd('opponent'), 0))*0.14*a.mhp) / (0.09 * f.ntd('opponent') + 0.02 * f.ntd('friend') + 1/(f.ntd('opponent') + 0.2))]>
//swo <aiPos:[(1 + 0.2 * ([2,3].contains(tr) ? (tr - 1) : 0)) / (0.5 + 0.25 * f.ntd('opponent')) / (f.ntd('opponent') < 6 ? (1 + 0.03 * Math.pow(Math.max(f.ntd('friend') - 2, 0),2)) : 1),
//   (a.mhp/(a.hp + 1)) * ((f.isf('enemyFort') + f.isf('actorFort')) * Math.max(Math.min(a.mhp - 1.6 * a.hp, 0.1 * a.hp), 0) + (f.isf('actorFort') + f.isf('enemyFort') * 0.4 * Math.max(4 - f.ntd('opponent'), 0))*0.2*a.mhp) / (0.5 + 0.25 * f.ntd('opponent')) / (f.ntd('opponent') < 6 ? (1 + 0.03 * Math.pow(Math.max(f.ntd('friend') - 2, 0),2)) : 1)]>
//  gen <aiPos:[(1 + 0.25 * ([2,3].contains(tr) ? (tr - 1) : 0)) / (0.5 + 0.25 * f.ntd('opponent')) / (f.ntd('opponent') < 6 ? (1 + 0.02 * Math.pow(Math.max(f.ntd('friend') - 3, 0),2)) : 1),
//(a.mhp/(a.hp + 1)) * ((f.isf('enemyFort') + f.isf('actorFort')) * Math.max(Math.min(a.mhp - 2 * a.hp, 0.1 * a.hp), 0) + (f.isf('actorFort') + f.isf('enemyFort') * 0.4 * Math.max(4 - f.ntd('opponent'), 0))*0.2*a.mhp) / (0.5 + 0.25 * f.ntd('opponent')) / (f.ntd('opponent') < 6 ? (1 + 0.02 * Math.pow(Math.max(f.ntd('friend') - 3, 0),2)) : 1)]>
 // 