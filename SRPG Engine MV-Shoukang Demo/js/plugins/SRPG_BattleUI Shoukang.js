...
/*:
* @plugindesc SRPG Battle UI adjustment, edited by Shoukang to support battlePrepare plugin compatibility
* @author SRPG Team
*
* @help
...
(function () {
  ...
  Scene_Menu.prototype.createStatusWindow = function() {
    ...
    if ($gameSystem.isSRPGMode()) {
      //no need for this if(), instead, change the statusWindow position when trigger commandformation and onformationcancel in battleprepare phase
  ...
  Scene_Menu.prototype.commandFormation = function () {
    ...
    // TODO: change the position here
    //if($gameSystem.isSRPGMode() == true && $gameSystem.isBattlePhase() == 'battle_prepare') this._statusWindow.x = (Graphics.boxWidth - this._statusWindow.width)/2;
    this.showStatusAndHideCommand();
    if (this._SrpgMemberRequirementWindow) this._SrpgMemberRequirementWindow.show()
  };
...
Window_MenuCommand.prototype.addMainCommands = function() {
    ...
    if (this.needsCommand('item') && !$gameSystem.isSRPGMode()) {
        this.addCommand(TextManager.item, 'item', enabled);
    }
    if (this.needsCommand('skill')  && !$gameSystem.isSRPGMode()) {
        this.addCommand(TextManager.skill, 'skill', enabled);
    }
    if (this.needsCommand('equip')  && !$gameSystem.isSRPGMode()) {
        this.addCommand(TextManager.equip, 'equip', enabled);
    }
    if (this.needsCommand('status')  && !$gameSystem.isSRPGMode()) {
...
Window_MenuCommand.prototype.addFormationCommand = function() {
    ...
    if (this.needsCommand('formation') && !$gameSystem.isSRPGMode()) {
...
// Window Opacity ---------------------------------------------------------------
  ...
  Scene_Menu.prototype.onFormationCancel = function () {
    //In case not in srpg mode.
    if($gameSystem.isSRPGMode() == true && $gameSystem.isBattlePhase() == 'battle_prepare' && !$gameParty.isValidActorNumber()){
        SoundManager.playBuzzer();
        this._statusWindow.activate();
    } else{
      _Scene_Menu_onFormationCancel.call(this);
      // TODO: change the position back to normal
      //if($gameSystem.isSRPGMode() == true && $gameSystem.isBattlePhase() == 'battle_prepare') this._statusWindow.x = (Graphics.boxWidth - this._statusWindow.width)/2;
      this.showCommandAndHideStatus();
      if (this._SrpgMemberRequirementWindow) this._SrpgMemberRequirementWindow.hide()
    }
  };
  ...
  //This is where I set the window position, override this
  Scene_Menu.prototype.createSrpgMemberRequirementWindow = function() {
      if ($gameSystem.isSRPGMode() == true && $gameSystem.isBattlePhase() === 'battle_prepare'){
          ...
          //TODO: edit the this._SrpgMemberRequirementWindow.y position
          ...
          this._SrpgMemberRequirementWindow.hide()
      }
  };
