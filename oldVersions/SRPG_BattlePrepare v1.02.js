...
//=================================================================================================
	...
	Game_Player.prototype.SRPGBattlePrepareSelectActor = function(event){
		var battlerArray = $gameSystem.EventToUnit(event.eventId());
	...
	Game_Player.prototype.SRPGBattlePrepareSelectEnemy = function(event){
		var battlerArray = $gameSystem.EventToUnit(event.eventId());
		SoundManager.playOk();
	...
	Game_Player.prototype.SRPGBattlePrepareSelectEmptyActor = function(event){
		SoundManager.playOk();
		...
		if ($gameSystem.isSRPGMode() == true && !$gameMap.isEventRunning() && $gameSystem.isBattlePhase() === 'battle_prepare'){
			...
			if ($gameSystem.isSubBattlePhase() === 'normal' && triggers[0] === 0) {
				if (event.isType() === 'actor' && Number(event.event().meta.id) === 0 && !event.isErased()) {
					this.SRPGBattlePrepareSelectActor(event);
				} else if (event.isType() === 'enemy') {
					this.SRPGBattlePrepareSelectEnemy(event);
					...
					this.SRPGBattlePrepareSelectEmptyActor(event);
				} else SoundManager.playBuzzer();
...
//===================================================================================
//Helper functions to update call menu
//===================================================================================
