 ...
 *==========================================================================================================================
 * V1.03 Enjoy the new Appearance and features! Not compatible with the Battle UI plugin. Remove funtion includes() for better compatibility.
 ...
 * Not compatible with the current version of Battle UI plugin. (7/13/2021)
 */
...
//=================================================================================================
	...
	Game_Player.prototype.srpgBattlePrepareExchangePosition = function(event){
		...
		} else if (event.eventId() !== $gameTemp.activeEvent().eventId()) SoundManager.playBuzzer();
	}
		...
		if ($gameSystem.isSRPGMode() == true && !$gameMap.isEventRunning() && $gameSystem.isBattlePhase() === 'battle_prepare'){
			...
			if ($gameSystem.isSubBattlePhase() === 'normal' && triggers[0] === 0) {
				if (event.isType() === 'actor' && Number(event.event().meta.id) === 0 && !event.isErased()) {
					...
					if (!_disableActorCommand && $gameParty.canAddActor(actorId)) $gameParty.srpgBattlePrepareSelectEmptyActor(event, actorId);
...
//=============================================================================
	Game_System.prototype.srpgStartBattlePrepare = function() {
		...
		this.setSrpgPreparePhaseOpenMenu(true);
	};
	...
	Scene_Menu.prototype.onFormationCancel = function() {
	    if ($gameSystem.isSRPGMode() == true && $gameSystem.isBattlePhase() == 'battle_prepare'){
	    	if ($gameParty.isValidActorNumber()){
	    		...
	    		_shoukang_Scene_Menu_onFormationCancel.call(this);
	    	} else {
